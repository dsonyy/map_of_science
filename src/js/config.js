// Article configuration
export const PATH_TO_ARTICLES = "articles/";

// Chart configurations
export const CITY_SIZE_THRESHOLD_0 = 50;
export const CITY_SIZE_THRESHOLD_1 = 200;
export const CITY_SIZE_THRESHOLD_2 = 500;
export const CITY_SIZE_THRESHOLD_3 = 1000;
export const CITY_SIZE_THRESHOLD_4 = 2000;
export const CITY_SIZE_THRESHOLD_5 = Infinity;
export const CITY_5_INNER_SIZE = 8;
export const CITY_5_OUTER_SIZE = 14;

// Foreground layer configurations
export const LAYER_ZOOM_THRESHOLD_0 = -Infinity;
export const LAYER_ZOOM_THRESHOLD_1 = 0.8;
export const LAYER_ZOOM_THRESHOLD_2 = 3.2;

export const LAYER_ZOOM_RADIUS_0 = LAYER_ZOOM_THRESHOLD_0;
export const LAYER_ZOOM_RADIUS_1 = LAYER_ZOOM_THRESHOLD_1;
export const LAYER_ZOOM_RADIUS_2 = LAYER_ZOOM_THRESHOLD_2;

// Labels
export const LABEL_ZOOM_SCALE_FACTOR_MIN = 0.5;
export const LABEL_ZOOM_SCALE_FACTOR_MAX = 1.5;
export const LABEL_ZOOM_SCALE_FACTOR_K = 0.5;
